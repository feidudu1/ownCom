...
export const shaderFs = `
  ...
  void main() {
    ...
    for (float delta = 0.; delta < n; delta++) {
      ...
      vec4 texR1 = texture(tDiffuse, saturate(uv1 + d1r));
      vec4 texR2 = texture(tDiffuse, saturate(uv1 - d1r));
      vec4 texG1 = texture(tDiffuse, saturate(uv2 + d1g));
      vec4 texG2 = texture(tDiffuse, saturate(uv2 - d1g));
      vec4 texB1 = texture(tDiffuse, saturate(uv3 + d1b));
      vec4 texB2 = texture(tDiffuse, saturate(uv3 - d1b));
      ...
      vec4 texR3 = texture(tDiffuse, saturate(uv1 + d2r));
      vec4 texR4 = texture(tDiffuse, saturate(uv1 - d2r));
      vec4 texG3 = texture(tDiffuse, saturate(uv2 + d2g));
      vec4 texG4 = texture(tDiffuse, saturate(uv2 - d2g));
      vec4 texB3 = texture(tDiffuse, saturate(uv3 + d2b));
      vec4 texB4 = texture(tDiffuse, saturate(uv3 - d2b));
    ...
    gl_FragColor = vec4(3.0, 2.0, 4.0, 1.0);
  }
