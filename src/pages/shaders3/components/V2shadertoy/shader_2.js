...
Surface map(vec3 p) {
  ...
  vec3 fcl = texture2DLodEXT(tNoise, p.xz * 0.1, 0.).rgb * (_checker(p.xz, vec2(2.5)) * .0625 + .02);
  return near(
...
vec3 lighting( in Hit h) {
  if (h.ray.len > MAX_TRACE_DIST) return pow(texture2DLodEXT(tDiffuse, -h.ray.dir, 0.).rgb, vec3(0.4)) * .25;
...
vec4 render( in Ray r) {
  ...
  // cheap trick
  c = h0.ray.col;
  if (len2(c) >= .2) col += texture2DLodEXT(tDiffuse, -h0.ray.dir, 0.).rgb * c * c * 3.0;
