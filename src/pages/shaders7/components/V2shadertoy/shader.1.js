...
bool RayIntersectSphere (inout SSphere sphere, inout SCollisionInfo info, in vec3 rayPos, in vec3 rayDir, in int ignorePrimitiveId)
{
	...
	float A = texture2D(tDiffuse, info.m_textureCoordinates + vec2(0.0, 0.0)).x;
	float B = texture2D(tDiffuse, info.m_textureCoordinates + vec2(delta, 0.0)).x;
    float C = texture2D(tDiffuse, info.m_textureCoordinates + vec2(0.0, delta)).x;
...
bool RayIntersectAABox (inout SAxisAlignedBox box, inout SCollisionInfo info, in vec3 rayPos, in vec3 rayDir, in int ignorePrimitiveId)
{
	...
	float delta = -1.0/512.0;
	if (box.m_Id == 4) {
		float A = texture2D(tNoise, info.m_textureCoordinates + vec2(0.0, 0.0)).x;
		float B = texture2D(tNoise, info.m_textureCoordinates + vec2(delta, 0.0)).x;
    	float C = texture2D(tNoise, info.m_textureCoordinates + vec2(0.0, delta)).x;
		vec3 norm = normalize(vec3(B - A, C - A, 0.25));
		info.m_surfaceNormal = normalize(mattanspace * norm);
	}
	else {
	...
	for(int index = 0; index < c_maxRayBounces; ++index)
	{
		...
		{			
			if (collisionInfo.m_Id == 4)
				collisionInfo.m_material.m_diffuseColor = texture2D(tNoise, collisionInfo.m_textureCoordinates).xyz;
			else
				collisionInfo.m_material.m_diffuseColor = texture2D(tDiffuse, collisionInfo.m_textureCoordinates).xyz;
